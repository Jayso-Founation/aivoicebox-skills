...
def on_message(client, userdata, msg):
    ...
    if msg.topic == _SUB_INTENT_ON_SUCCESS:
        ...
        if intent == "Weather":
            ...
            # City name detected
            city = nlu_payload["slots"][0]["rawValue"]

            # Call weather api to get weather data for detected city
            w_data = json.loads(requests.get("http://api.weatherapi.com/v1/current.json?key=6e1165d7a012466fa40142441222802&q="+city+"&aqi=no").content)

            # Get temperature in celsius
            temp_c = w_data['current']['temp_c']

            # Create sentence for aivoicebox to speak
            sentence = "Weather in " + city + " is " + str(temp_c)
